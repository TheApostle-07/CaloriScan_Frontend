...
const QRScanner = () => {
  ...
  useEffect(() => {
    const fetchTokenStatus = async () => {
      const response = await axios.get("https://calori-scan-backend-iy7x.vercel.app/api/token-status");
  ...
  const handleCaptureImage = async () => {
    if (videoRef.current) {
      ...
      try {
        ...
        const response = await axios.post("https://calori-scan-backend-iy7x.vercel.app/api/detect", {
